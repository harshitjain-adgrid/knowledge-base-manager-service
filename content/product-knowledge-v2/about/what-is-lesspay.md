---
title: What LessPay Does for Your Shop
type: concept
tags: [overview, getting-started, payments]
audience: merchant
status: published
owner: product-team
last_reviewed: 2026-08-27
review_by: 2027-02-27
answers:
  - what LessPay is and what it does for a shop
  - whether LessPay holds your money
  - what LessPay charges a merchant for
not_covered:
  - the steps to join LessPay and set up an account
  - what the assistant inside the app can do
related: [what-chotu-can-do, how-you-join-lesspay, how-you-get-paid, what-promotions-are]
aliases: [lesspay kya hai, yeh app kya karta hai, about lesspay]
entities: [LessPay Business, UPI, QR]
---

# What LessPay Does for Your Shop

LessPay puts your shop in front of people nearby, lets them pay you by UPI, and
gives them a discount you decide. You keep your own UPI account — LessPay never
holds your money.

There are two apps. You use **LessPay Business**. Your customers use the LessPay
customer app to find shops near them.

## What you get out of it

Three things, in the order most shopkeepers care about them:

- **You get found.** Your shop appears in the customer app for people searching
  or browsing nearby, with your photos, timings and whatever you are known for.
- **You get paid.** Customers scan your UPI QR in the shop and pay. The money
  goes to your bank account on the normal settlement schedule.
- **You bring people in.** You set discounts and deals that customers can see
  before they walk in, and you can pay to be shown to more people.

## LessPay never holds your money

Payments settle straight through to your bank account. LessPay is not a wallet
sitting in the middle holding your takings — it splits the payment at the moment
it happens.

That is why your settlement lands on a schedule rather than instantly: it is the
banking system moving it, not LessPay releasing it.

## What LessPay charges for

There is no subscription for the basics. Setting up your shop profile, being
listed in the customer app and running your own offers cost you nothing, however
many offers you run.

Two things you choose to buy:

- **Packages and programs** you subscribe to, which give you credits and other
  benefits every month.
- **Promotions** — paying to have your shop or offer shown to more customers
  inside the customer app.

Separately from those, **a fee is deducted from each payment** before the money
settles to your bank. That is not something you subscribe to; it comes out of
the payment itself. Every payment's receipt shows both figures side by side —
what the customer paid, and what you keep after the fee — so the exact amount
for any payment is on that payment's receipt.

## What if something is not working the way you expect

Almost everything in LessPay comes back to one of four things: your shop
profile, your offers, your QR, or your wallet. Working out which of the four a
problem belongs to usually points straight at the answer.

Chotu can also just be asked. Describing the problem in your own words —
English or Hinglish — is enough; you do not need to know what the feature is
called.

## Hinglish mein

LessPay aapki dukaan ko aas paas ke logon tak pahunchata hai, unse UPI se paisa
dilwata hai, aur aap jo chhoot dena chahein wo unhein dikhata hai. Do app hote
hain: aap LessPay Business chalate hain aur customer LessPay ka customer app.
Sabse zaroori baat yeh hai ki LessPay aapka paisa apne paas nahi rakhta — paisa
seedha aapke apne UPI account mein jaata hai aur bank ke schedule par pahunchta
hai. Dukaan ka profile banana, app mein dikhna aur apne offer chalana bilkul
free hai — iska koi subscription nahi hai. Paise aap do cheezon ke dete hain:
package ya program lene par, aur promotion par jab zyada logon tak pahunchna ho.
In dono se alag, har payment par ek fee kat ti hai jo settle hone se pehle nikal
jaati hai. Har payment ki receipt par dono cheezein likhi hoti hain — customer
ne kitna diya aur fee ke baad aapke paas kitna bacha.

## Frequently asked as

- "what is lesspay"
- "yeh app kya karta hai"
- "यह ऐप क्या करता है"
- "how does lesspay work for my shop"
- "lesspay se paisa kaise aata hai"
- "लेसपे से पैसा कैसे आता है"
- "do you keep my money"
- "what do I have to pay for"
- "how do customers find my shop on lesspay"
